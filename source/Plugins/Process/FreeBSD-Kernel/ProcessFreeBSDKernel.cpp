...
lldb::ProcessSP
ProcessFreeBSDKernel::CreateInstance(Target &target, Listener &listener, const FileSpec *crash_file_path)
{
    lldb::ProcessSP process_sp;
    // Check if the target is potentially a FreeBSD kernel image
    lldb_private::SymbolVendor *sym_vendor = target.GetExecutableModule()->GetSymbolVendor();
...
//----------------------------------------------------------------------
ProcessFreeBSDKernel::ProcessFreeBSDKernel(Target& target, Listener &listener, const FileSpec &crash_file_path)
...
Error
ProcessFreeBSDKernel::DoLaunch (Module *exe_module, ProcessLaunchInfo &launch_info)
{
    Error error;
    error.SetErrorString("launching not supported in FreeBSD-kernel plug-in");
    return error;
...
lldb::addr_t ProcessFreeBSDKernel::LookUpSymbolAddressInModule(lldb::ModuleSP module, const char *name)
{
...
ProcessFreeBSDKernel::AddProcs(uintptr_t paddr)
{
    ...
    while (paddr != 0)
    {
        ...
        while (addr != 0)
        {
            if (kvm_read(m_kvm, addr, &td, sizeof(td)) != sizeof(td))
            {
                if (log)
                    log->Printf("kvm_read: %s", kvm_geterr(m_kvm));
                break;
            ...
            ThreadSP thread_sp;
            ThreadFreeBSDKernel * kthread = CreateNewThreadFreeBSDKernel(*this, td.td_tid);

            kthread->m_kaddr = addr;
            if ((lldb::tid_t)td.td_tid == m_dumptid)
                kthread->m_pcb = m_dumppcb;
            else if (TD_IS_RUNNING(&td) && CPU_ISSET(td.td_oncpu, &m_stopped_cpus))
            ...
            addr = reinterpret_cast<addr_t>(TAILQ_NEXT(&td, td_plist));
        }
        paddr = reinterpret_cast<addr_t>(LIST_NEXT(&p, p_list));
    }
}

addr_t
ProcessFreeBSDKernel::FindCorePCB(uint32_t cpuid)
{
    ModuleSP module = GetTarget().GetExecutableModule();
